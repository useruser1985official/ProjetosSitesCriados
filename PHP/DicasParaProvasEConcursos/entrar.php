<?php
    ...
    $login = isset($_POST["login"]) ? trim(htmlspecialchars(strip_tags($_POST["login"]), ENT_QUOTES)) : "";
    $senha = isset($_POST["senha"]) ? trim(htmlspecialchars(strip_tags($_POST["senha"]), ENT_QUOTES)) : "";
    $lemb = isset($_POST["lemb"]) ? trim(htmlspecialchars(strip_tags($_POST["lemb"]), ENT_QUOTES)) : "";
    ...
    if($login == $row["login"] && $crip == $row["senha"]) {
        ...
        session_set_cookie_params($exp, "DicasParaProvasEConcursos", "localhost");
        session_start();

        $_SESSION["DicasParaProvasEConcursos"]["logado"] = true;
        
        header("location: consultas.php");
    }
    else if($login == $row["login"] && $crip != $row["senha"]) {
        $_SESSION["DicasParaProvasEConcursos"]["logado"] = null;
