...
class AdminDAO {
    ...
    private function listar($login, $senha, $lemb, $row) {
        ...
        if($login == $row["login"] && $crip == $row["senha"]) {
            ...
            session_set_cookie_params($exp, "DicasDeDesenhoArtistico", "localhost");
            session_start();
            $_SESSION["DicasDeDesenhoArtistico"]["logado"] = true;
            header("location: consultas.php");
        }
        else if($login == $row["login"] && $crip != $row["senha"]) {
            $_SESSION["DicasDeDesenhoArtistico"]["logado"] = null;
