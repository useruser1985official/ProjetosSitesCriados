...
class AdminDAO {
    ...
    private function listar($login, $senha, $lemb, $row) {
        ...
        if($login == $row["login"] && $crip == $row["senha"]) {
            ...
            session_set_cookie_params($exp, "RevolucionariosDoAxe", "localhost");   
            session_start();
            $_SESSION["RevolucionariosDoAxe"]["logado"] = true;
            header("location: consultas.php");
        }
        else if($login == $row["login"] && $senha != $row["senha"]) {
            $_SESSION["RevolucionariosDoAxe"]["logado"] = null;
