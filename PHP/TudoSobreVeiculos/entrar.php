...
$login = htmlspecialchars(isset($_POST["tLogin"])) ? trim(htmlspecialchars(strip_tags($_POST["tLogin"], ENT_QUOTES))) : "";
$senha = htmlspecialchars(isset($_POST["tSenha"])) ? trim(htmlspecialchars(strip_tags($_POST["tSenha"], ENT_QUOTES))) : "";
$lemb = htmlspecialchars(isset($_POST["tLemb"])) ? trim(htmlspecialchars(strip_tags($_POST["tLemb"], ENT_QUOTES))) : "off";
...
if($login == $row["login"] && $crip == $row["senha"]) {
    ...
    session_set_cookie_params($exp, "TudoSobreVeiculos", "localhost");
    session_start();
    $_SESSION["TudoSobreVeiculos"]["logado"] = true;
    header("location: consultas.php");
}
else if($login == $row["login"] && $crip != $row["senha"]) {
    $_SESSION["TudoSobreVeiculos"]["logado"] = null;
